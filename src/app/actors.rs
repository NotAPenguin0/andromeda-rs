...
impl RootActorSystem {
    pub async fn new(gfx: &gfx::SharedContext) -> Result<Self> {
        ...
        let scene_texture = system
            .create_actor("target_resize", TargetResizeActor::default())
            .await?;

        let camera = system
            .create_actor("camera_state", state::Camera::default())
            .await?;
        ...
        let camera_controller = system
            .create_actor(
                "camera_controller",
                CameraController::new(input.clone(), camera.clone()),
            )
            .await?;

        input.tell(AddInputListener(CameraScrollListener::new(
    ...
    pub async fn update_rt_size(
        ...
        if let Some(size) = size {
            ...
            let old = self
                .scene_texture
                .ask(gui::QueryCurrentSceneTexture)
                .await?;
            if let Some(old) = old {
